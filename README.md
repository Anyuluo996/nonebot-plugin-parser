...
> 本节命令名跟随 `parser_force_prefix`(未配置默认 `par`),下文以 `par` 为例。
...
| 受控项 | 对应命令 | 说明 |
| --- | --- | --- |
| `强制解析` | 前缀强制解析(`par+链接`) | 平台被群管关闭后,授权用户可绕过 |
| `bilibili登录` | `blogin` | B 站扫码登录(仅私聊) |
| `网易云登录` | `par网易云登录`/`parwyy登录`/登出 | 网易云凭据管理 |
...
默认 `dycookie`/`parqq登录` 等凭据命令仅 SUPERUSER 可用。SUPERUSER 可授权指定用户代为维护凭据:

```
par全局授权 @运维 抖音cookie
```

运维即可在任何群/私聊执行 `dycookie <值>` / `dycookie查看`,无需 SUPERUSER 身份。
