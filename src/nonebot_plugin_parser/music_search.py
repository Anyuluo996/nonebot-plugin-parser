...
| 服务   | 接口                              | id 字段 | 歌名字段     | 歌手字段            |
| ------ | --------------------------------- | ------- | ------------ | ------------------- |
| 网易云 | ``/api/search/get/web``           | ``id``  | ``name``     | ``artists[].name``  |
| QQ音乐 | ``qqmusic_api.general_search``    | ``mid`` | ``name``     | ``singer[].name``   |
| 酷狗   | ``/api/v3/search/song``           | ``hash``| ``songname`` | ``singername``      |
"""
...
async def search_netease(
    ...
    try:
        resp = await parser.request(
            _NETEASE_SEARCH_API,
            params={"s": keyword, "type": 1, "offset": 0, "limit": limit, "httpStatus": 1},
...
async def search_kugou(parser: "BaseParser", keyword: str, limit: int = DEFAULT_PER_SERVICE_LIMIT) -> list[SearchItem]:
    ...
    try:
        resp = await parser.request(
            _KUGOU_SEARCH_API,
            params={"keyword": keyword, "page": 1, "pagesize": limit, "showtype": 10},
