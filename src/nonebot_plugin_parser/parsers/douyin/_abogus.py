"""抖音 Web API ``a_bogus`` 签名参数生成器。

本模块被 ``DouyinParser.parse_slides`` 调用, 为 PC web 详情接口
( ``/aweme/v1/web/aweme/detail/`` ) 的请求参数生成 ``a_bogus`` 签名。
没有该签名, 即使带登录态 ``ttwid`` 抖音也会返回 200 + 空 body (风控),
导致图文/实况照片 (live photo) 的视频轨无法解析。
...
算法实现来自开源项目 (保留原始版权声明):
    ...
    Modified by:
        And this file is now a part of the
        https://github.com/Evil0ctal/Douyin_TikTok_Download_API open-source project.
        This project is licensed under the Apache License 2.0, and the original
        author information is kept.
    Purpose:
        This file is used to generate the `a_bogus` parameter for the Douyin Web API.

依赖 ``gmssl`` 提供国密 SM3 哈希 (a_bogus 内部用 SM3 而非 MD5/SHA)。
...
典型用法::
    ...
    params["a_bogus"] = a_bogus  # 追加到 query string
"""
...
class ABogus:
    ...
    实例可复用 (无不可变状态残留, ``get_value`` 内部会 reset)。
    """
    ...
    @staticmethod
    def random_list(a: float | None = None, b=170, c=85, d=0, e=0, f=0, g=0):
        r = a or (random() * 10000)
    ...
    def get_value(
        ...
        Args:
            url_params: 请求参数。dict 则内部 urlencode, str 则直接用。
            method: HTTP 方法, 默认 GET。
        Returns:
            未 url 编码的 a_bogus 字符串 (调用方需自行 quote)。
        """
