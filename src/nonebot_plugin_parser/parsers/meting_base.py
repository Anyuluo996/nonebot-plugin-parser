...
class MetingBaseParser(BaseParser):
    ...
    async def _parse_by_song_id(self, song_id: str, share_url: str, include_cover: bool = False):
        ...
        include_cover=True 时（网易云）将歌曲封面下载任务塞进
        ``ParseResult.cover_image``：仅供渲染器画卡片，发送流程不读取它，
        因此封面只进卡片、不会被当作独立图片消息发出。音频始终是 audio。
        """
        ...
        if include_cover:
            ...
            if cover_url:
                cover_image = self.create_cover_image_task(cover_url)
