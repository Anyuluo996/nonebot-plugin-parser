...
class CommonRenderer(ImageRenderer):
    ...
    async def _render_grid(
        ...
        ctx: RenderContext,
        contents: list,
        fetch_path: Callable[[Any], Awaitable[Path | None]],
        *,
        draw_play_button: bool,
    ) -> None:
        """渲染媒体缩略图网格的通用逻辑。

        Args:
            ctx: 渲染上下文。
            contents: 媒体内容列表（``ImageContent`` / ``DynamicContent``）。
            fetch_path: 异步回调，从单个 content 取出待渲染的图片路径（None 表示跳过）。
            draw_play_button: 是否在每个缩略图上叠加视频播放按钮（动图网格用）。
        """
        total = len(contents)
