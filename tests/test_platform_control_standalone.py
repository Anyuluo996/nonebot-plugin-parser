...
async def test_multiple_groups(controller: PlatformController):
    ...
    # 清空并设置不同的群组配置
    # 注意：get_group_key 返回 "test_{group_id}"，所以键需要加上 "test_" 前缀
    controller.disabled_platforms.clear()
    controller.disabled_platforms["test_group_a"] = {"bilibili", "douyin"}
    controller.disabled_platforms["test_group_b"] = {"twitter", "youtube"}
    controller.disabled_platforms["test_group_c"] = set()
    controller._save_data()
...
async def test_private_chat(controller: PlatformController):
    ...
    # 群聊会话
    group_session = MockSession("group_789")
